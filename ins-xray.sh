...
/root/.acme.sh/acme.sh --issue -d $domain --standalone -k ec-256 --server letsencrypt >> /etc/xdev/tls/$domain.log
~/.acme.sh/acme.sh --installcert -d $domain --fullchainpath /etc/xdev/xray/xray.crt --keypath /etc/xdev/xray/xray.key --ecc
# Uuid Service
uuid=$(cat /proc/sys/kernel/random/uuid)
# INSTALL XRAY
wget -c -P /etc/xdev/xray/ "https://github.com/XTLS/Xray-core/releases/download/v1.4.5/Xray-linux-64.zip"
unzip -o /etc/xdev/xray/Xray-linux-64.zip -d /etc/xdev/xray 
rm -rf /etc/xdev/xray/Xray-linux-64.zip
chmod 655 /etc/xdev/xray/xray
# XRay boot service
...
NoNewPrivileges=yes
ExecStart=/etc/xdev/xray/xray run -confdir /etc/xdev/xray/conf
Restart=on-failure
...
CapabilityBoundingSet=CAP_NET_BIND_SERVICE CAP_NET_RAW
NoNewPrivileges=yes
ExecStart=/etc/xdev/xray/xray run -config /etc/xdev/xray/vless-nontls.json
Restart=on-failure
RestartPreventExitStatus=23
...
rm -rf /etc/xdev/xray/conf/*
cat <<EOF >/etc/xdev/xray/conf/00_log.json
{
  "log": {
...
EOF
cat <<EOF >/etc/xdev/xray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
cat <<EOF >/etc/xdev/xray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
cat <<EOF >/etc/xdev/xray/conf/02_VLESS_TCP_inbounds.json
{
  "inbounds": [
    {
      "port": 443,
      ...
      "streamSettings": {
        ...
        "xtlsSettings": {
          ...
          "certificates": [
            {
              "certificateFile": "/etc/xdev/xray/xray.crt",
              "keyFile": "/etc/xdev/xray/xray.key",
              "ocspStapling": 3600,
              "usage": "encipherment"
            }
          ]
...
EOF
cat <<EOF >/etc/xdev/xray/conf/03_VLESS_WS_inbounds.json
{
  "inbounds": [
    {
      "port": 31297,
...
EOF
cat <<EOF >/etc/xdev/xray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
cat <<EOF >/etc/xdev/xray/conf/04_trojan_TCP_inbounds.json
{
  "inbounds": [
    {
      "port": 31296,
...
EOF
cat <<EOF >/etc/xdev/xray/conf/05_VMess_WS_inbounds.json
{
  "inbounds": [
    {
      "listen": "127.0.0.1",
...
EOF
cat <<EOF >/etc/xdev/xray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
cat > /etc/xdev/xray/conf/vmess-nontls.json << END
{
  "log": {
...
cat > /etc/xdev/xray/vless-nontls.json << END
{
  "log": {
